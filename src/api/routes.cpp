...
void register_library_routes(httplib::Server& srv, sqlite3* db) {
    srv.Get("/api/libraries", [db](const httplib::Request&, httplib::Response& r) {
        ...
        if (all_syms)
            for (auto& s : *all_syms) used_libs.insert(s.library_id());
        ...
        if (comp_libs)
            for (auto& cl : *comp_libs) group_name[cl.id] = cl.name;
        json arr = json::array();
        if (libs)
            for (auto& l : *libs) {
                if (l.name.find("_footprints") != std::string::npos) continue;
                if (!used_libs.count(l.id)) continue;
                json o;
                o["id"] = l.id;
                o["name"] = l.name;
    ...
    srv.Post("/api/libraries", [db](const httplib::Request& req, httplib::Response& r) {
        json j;
        try {
            ...
            if (action == "delete") {
                ...
                if (libs)
                    for (auto& l : *libs)
                        if (l.id == id) file_path = l.file_path.string();
                int removed = 0;
                ...
                j["ok"] = db_r.has_value();
                if (file_deleted) j["deleted_file"] = file_path;
                ...
                if (!sym) { j["ok"] = false; j["error"] = "Not found"; }
                else {
                    if (sr.remove(sym_id)) {
                        ...
                        if (libs)
                            for (auto& l : *libs) {
                                if (l.id != sym->library_id() || l.file_path.empty()) continue;
                        ...
                        j["ok"] = true;
                    } else { j["ok"] = false; j["error"] = "DB remove failed"; }
                }
        ...
        } catch (...) { j["ok"] = false; j["error"] = "Parse error"; }
...
void register_symbol_routes(httplib::Server& srv, sqlite3* db) {
    srv.Get("/api/symbols", [db](const httplib::Request& req, httplib::Response& r) {
        ...
        try {
            ...
            if (!ov.empty()) offset = std::stoi(ov);
        } catch (...) {}
    ...
    srv.Get("/api/footprints", [db](const httplib::Request&, httplib::Response& r) {
...
void register_plugin_routes(httplib::Server& srv, plugin::PluginManager* plugins) {
    srv.Get("/api/plugins", [plugins](const httplib::Request&, httplib::Response& r) {
        json arr = json::array();
        for (auto& m : plugins->available_plugins()) {
            ...
            for (auto& a : m.actions) { json act; act["id"]=a.id; act["name"]=a.name; act["trigger"]=a.trigger; act["schema"]=a.schema; if (a.button_show) { json btn; btn["show"]=a.button_show; btn["style"]=a.button_style; btn["location"]=a.button_location; btn["tooltip"]=a.button_tooltip; act["button"]=btn; } actions.push_back(act); }
            p["actions"] = actions;
            arr.push_back(p);
        }
        r.set_content(arr.dump(), "application/json");
    ...
    srv.Post("/api/plugins/execute", [plugins](const httplib::Request& req, httplib::Response& r) {
        json j;
        try {
            ...
            if (!result) { j["ok"]=false; j["error"]=result.error().message; r.set_content(j.dump(),"application/json"); return; }
            auto out = json::parse(*result);
            j["plugin_result"] = out;
            j["ok"] = out.value("ok",false);
        } catch (...) { j["ok"]=false; j["error"]="Execute error"; }
        r.set_content(j.dump(),"application/json");
    });

    srv.Post("/api/plugins/load", [plugins](const httplib::Request& req, httplib::Response& r) {
        json j;
        try {
            auto body = json::parse(req.body.empty()?"{}":req.body);
            std::string id = body.value("id","");
            if (!id.empty()) { auto res = plugins->load(id,nullptr); j["ok"]=res.has_value(); if (!res) j["error"]=res.error().message; }
            else j["ok"]=false;
        } catch (...) { j["ok"]=false; }
        r.set_content(j.dump(),"application/json");
    });
    ...
    // Plugin icon routes
    srv.Get(R"(/api/plugins/([^/]+)/icon/([^/]+))", [plugins](const httplib::Request& req, httplib::Response& r) {
        auto pid = req.matches[1]; auto fn = req.matches[2];
    ...
    srv.Get(R"(/api/plugins/([^/]+)/icon)", [plugins](const httplib::Request& req, httplib::Response& r) {
        auto pid = req.matches[1]; auto pp = plugins->plugin_path(pid.str());
        for (auto& fn : {"icon.svg","icon.png"}) {
            auto icon_path = pp / fn;
            if (std::filesystem::exists(icon_path)) {
                std::ifstream f(icon_path, std::ios::binary);
                std::string data((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
                r.set_content(data, (std::string(fn)=="icon.svg")?"image/svg+xml":"image/png");
                return;
            }
        }
        r.status = 404;
...
void register_settings_routes(httplib::Server& srv, sqlite3* db) {
    srv.Get("/api/settings", [db](const httplib::Request&, httplib::Response& r) {
    ...
    srv.Post("/api/settings", [db](const httplib::Request& req, httplib::Response& r) {
        storage::SettingsRepository repo(db);
        try {
            ...
            if (has_sym || has_fp) {
                ...
                if (sym_path && !sym_path->empty()) {
                    auto r2 = services::ImportPipeline(db)
                        | services::symbols_from{*sym_path, ""}
                        | services::execute;
                ...
                if (fp_path && !fp_path->empty() && *fp_path != (sym_path?*sym_path:"")) {
                    auto r2 = services::ImportPipeline(db)
                        | services::footprints_from{*fp_path}
                        | services::execute;
                    total_sym += r2.symbols;
                    total_fp += r2.footprints;
                }
                if (m3d_path && !m3d_path->empty()) {
                    auto r2 = services::ImportPipeline(db)
                        | services::models_from{*m3d_path}
                        | services::execute;
                    total_m3d = r2.models_3d;
                }
                out["imported_symbols"]=total_sym; out["imported_footprints"]=total_fp; out["imported_models_3d"]=total_m3d;
                r.set_content(out.dump(),"application/json");
                return;
            }
            json out; out["ok"]=true; out["imported_symbols"]=0; out["imported_footprints"]=0;
            r.set_content(out.dump(),"application/json");
        } catch (...) {
            json out; out["ok"]=false; out["error"]="Save error";
            r.set_content(out.dump(),"application/json");
        }
    });

    srv.Get("/api/component-libraries", [db](const httplib::Request&, httplib::Response& r) {
        ...
        if (libs) for (auto& l : *libs) {
            ...
            o["symbol_path"]=l.symbol_path; o["footprint_path"]=l.footprint_path;
            o["model_3d_path"]=l.model_3d_path;
            o["enabled"]=l.enabled; o["sort_order"]=l.sort_order;
            arr.push_back(o);
        }
        r.set_content(arr.dump(),"application/json");
    });

    srv.Post("/api/component-libraries", [db](const httplib::Request& req, httplib::Response& r) {
        storage::ComponentLibraryRepository repo(db);
        json j;
        try {
            auto body = json::parse(req.body.empty()?"{}":req.body);
            ...
            if (action=="add") {
                ...
                lib.id=short_uuid(); lib.name=body.value("name","New Library");
                lib.symbol_path=body.value("symbol_path",""); lib.footprint_path=body.value("footprint_path","");
                lib.model_3d_path=body.value("model_3d_path",""); lib.enabled=body.value("enabled",true);
                auto ins=repo.insert(lib);
                j["ok"]=ins.has_value();
                if (ins) j["library"]={{"id",ins->id},{"name",ins->name}};
                else j["error"]=ins.error().message;
            } else if (action=="update") {
                std::string id=body.value("id","");
                if (id.empty()) { j["ok"]=false; j["error"]="Missing id"; }
                else {
                    storage::ComponentLibraryRepository::ComponentLibrary lib;
                    lib.id=id; lib.name=body.value("name","");
                    lib.symbol_path=body.value("symbol_path",""); lib.footprint_path=body.value("footprint_path","");
                    lib.model_3d_path=body.value("model_3d_path",""); lib.enabled=body.value("enabled",true);
                    auto upd=repo.update(lib); j["ok"]=upd.has_value();
                    if (!upd) j["error"]=upd.error().message;
                }
            } else if (action=="remove") {
                std::string id=body.value("id","");
                if (id.empty()) { j["ok"]=false; j["error"]="Missing id"; }
                else { auto rem=repo.remove(id); j["ok"]=rem.has_value(); if (!rem) j["error"]=rem.error().message; }
            } else if (action=="import") {
                auto result = services::ImportPipeline(db)
                    | services::symbols_from{"", ""}
                    | services::execute;
                j["ok"]=true; j["imported_symbols"]=result.symbols;
                j["imported_footprints"]=result.footprints; j["imported_models_3d"]=result.models_3d;
                j["auto_linked"]=result.linked_symbols; j["libraries"]=json::array();
            } else { j["ok"]=false; j["error"]="Unknown action: "+action; }
        } catch (...) { j["ok"]=false; j["error"]="Parse error"; }
        r.set_content(j.dump(),"application/json");
    });

    srv.Post("/api/db/reset", [db](const httplib::Request&, httplib::Response& r) {
        json j;
...
void register_type_routes(httplib::Server& srv, sqlite3* /*db*/) {
    srv.Get("/api/component-types", [](const httplib::Request&, httplib::Response& r) {
        auto& reg = core::TypeRegistry::instance();
        json arr = json::array();
        for (auto& ct : reg.component_types()) {
            json o; o["name"]=ct.name; o["icon"]=ct.icon; arr.push_back(o);
        }
        r.set_content(arr.dump(),"application/json");
    });
    srv.Post("/api/component-types", [](const httplib::Request& req, httplib::Response& r) {
        auto& reg = core::TypeRegistry::instance();
        json j;
        try {
            auto body = json::parse(req.body.empty()?"{}":req.body);
            ...
            if (action=="add" && !name.empty()) {
                core::TypeEntry e; e.name=name; e.icon=body.value("icon","");
    ...
    srv.Get("/api/package-types", [](const httplib::Request&, httplib::Response& r) {
        auto& reg = core::TypeRegistry::instance();
        json arr = json::array();
        for (auto& pt : reg.package_types()) {
            json o; o["name"]=pt.name; o["category"]=pt.category; arr.push_back(o);
        }
        r.set_content(arr.dump(),"application/json");
    });
    srv.Post("/api/package-types", [](const httplib::Request& req, httplib::Response& r) {
        auto& reg = core::TypeRegistry::instance();
        json j;
        try {
            auto body = json::parse(req.body.empty()?"{}":req.body);
            ...
            if (action=="add" && !name.empty()) {
                core::TypeEntry e; e.name=name; e.category=body.value("category","Other");
...
void register_status_routes(httplib::Server& srv, sqlite3* db) {
    srv.Get("/api/status", [db](const httplib::Request&, httplib::Response& r) {
        json j;
        storage::SymbolRepository sr(db);
        j["symbols"] = sr.count();
    ...
    srv.Post("/api/bye", [](const httplib::Request&, httplib::Response& r) {
        r.set_content(R"({"ok":true})", "application/json");
    });

    srv.Post("/api/pick-folder", [](const httplib::Request&, httplib::Response& r) {
        auto path = platform::NativeFolderDialog::pick_folder();
        json j;
        if (!path.empty()) { j["ok"]=true; j["path"]=path; }
        else { j["ok"]=false; j["path"]=""; }
        r.set_content(j.dump(),"application/json");
    });

    srv.Get("/api/rules", [](const httplib::Request&, httplib::Response& r) {
        auto rules = classifier::RuleLoader::default_rules();
        json arr = json::array();
        for (auto& rule : rules) {
            json o; o["name"]=rule.name; o["target"]=rule.target_library;
            o["confidence"]=rule.confidence; arr.push_back(o);
        }
        r.set_content(arr.dump(),"application/json");
    });
}
...
void register_classify_routes(httplib::Server& srv, sqlite3* db) {
    srv.Post("/api/import", [db](const httplib::Request& req, httplib::Response& r) {
        std::string dir = req.get_param_value("dir");
        json j;
        if (dir.empty()) { j["error"]="Missing 'dir' parameter"; }
        else {
            auto result = services::ImportPipeline(db)
                | services::symbols_from{dir, ""}
                | services::execute;
            j["ok"]=true; j["symbols"]=result.symbols;
            j["footprints"]=result.footprints; j["errors"]=0;
        }
        r.set_content(j.dump(),"application/json");
    });
}
