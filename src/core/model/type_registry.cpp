...
util::Result<void> TypeRegistry::build() {
    auto& r = instance();
    if (!r.load_component_types("config/component_types.json"))
        kforge::util::log_warn{}("config/component_types.json not found — using defaults");
    if (!r.load_package_types("config/package_types.json"))
        kforge::util::log_warn{}("config/package_types.json not found — using defaults");
    return {};
}
